...
@SCDoc
(
	description = "Find keys of objects in the cache that match the given regular expression",
	parameters =
	{
		"expression",		"Regex of the keys to find"
	}
)
public class SCCommandRegexFindKeys implements SCCommand
{
	SCCommandRegexFindKeys()
	{
	}
	...
	public SCDataBuilder newBuilder()
	{
		return new SCDataBuilder()
		{
			...
			public void executeCommand(SCServer server, SCConnection connection) throws Exception
			{
				...
				List<String>		keys = server.regExFindKeys(fKey);
